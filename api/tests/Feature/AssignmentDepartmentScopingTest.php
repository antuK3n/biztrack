...
/*
 * `scopedAssignmentFiling()` and `choAssignmentId()` used to live in
 * AssignmentDepartmentScopingTest.php. They moved to tests/Pest.php when
 * OfficerInChargeTest needed the same filing — a Pest run filtered to one file
 * loads only that file, so a fixture two suites share has to be in the shared
 * home rather than in whichever suite happened to write it first.
 */
