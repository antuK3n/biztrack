...
/**
 * CPDD's Locational Clearance sheet, read-only, for a host that is not the
 * clearance stage.
 *
 * ── Why an export rather than a copy ──────────────────────────────────────
 *
 * An amendment that moves the premises applies for a fresh Zoning Clearance
 * as part of the filing, and the client asked for that sheet to appear inside
 * the amendment form: *"we already have the application form in the system,
 * just copy its contents and bring them in the said section."*
 *
 * Copying it is what this export exists to avoid. The same instinct produced
 * a second trade picker earlier the same day, and the copy had drifted from
 * the original — different icon size, different placeholder, different
 * empty-state wording — before it was ever shown to anyone. One component,
 * two hosts, no way to drift.
 *
 * ── Read-only, and why that is the honest shape ──────────────────────────
 *
 * MCG-CPDD-FO-003 is almost entirely DERIVED: proprietor, contact, email,
 * address, trade and area are all tagged "from application", because the
 * applicant answered them on the BPLO form and asking twice invites two
 * answers. There is nothing here for them to fill in, so presenting it as a
 * form to complete would be a form that completes itself.
 *
 * What it is FOR is showing the applicant exactly what the zoning officer
 * will read, off the values this amendment is asking for rather than the ones
 * still on the register — CPDD has to assess where the business is GOING.
 * Feeding it the register's copy would show them the address they are leaving,
 * which is the knot `WorkflowService::permitTypeIdsAtSubmission` already
 * describes from the other side.
 */
export function ZoningSheetPreview({ business }: { business: CarriedOverBusiness }) {
  return (
    <ReadOnlyContext.Provider value={true}>
      {/* No answer here is carried from last year: this sheet is being
          previewed, not resumed, so the tag would be claiming a history the
          preview does not have. */}
      <CarriedContext.Provider value={[]}>
        <ZoningFields data={{}} set={() => {}} business={business} />
      </CarriedContext.Provider>
    </ReadOnlyContext.Provider>
  )
}
