...
test('the filing shows on Track as awaiting BPLO, with nothing to pay yet', async ({ page }) => {
   ...
   *
   * It is the filing's STATUS now, in that status's own colour. A form
   * submitted an hour ago says "For Initial Approval", which is the answer to
   * the question the row is actually asked.
   *
   * The two payment words are still asserted absent, not merely unlooked-for:
   * a row drawing a status badge AND a stale payment claim is the failure
   * nobody checks for, and "Paid is absent" is a different statement from
   * "For Initial Approval is present".
   */
  await expect(
    row.getByText('For Initial Approval', { exact: true }),
    'a just-submitted filing should say BPLO is giving it the first approval',
  ).toBeVisible()
  await expect(
